...
year=0:15
cap_rate=c(seq(.06,.07,.001),rep(.07,5))
adj_cap_rate=cap_rate+c(.002,.003,.004,.005,rep(0,12))
noi=c(rep(10,4),rep(12,12))
value=noi/adj_cap_rate
cash_return=noi[-16]/value[1]
delta_value=diff(value)
tr=noi[-16]+delta_value
twr=tr/value[-16]
ncf=c(-value[1],noi[-16])
irrs=vector()
ncf=zoo(ncf,year)
value=zoo(value,year)
for (i in 1:15) {
  irrs[i]=irr.z(mergesum.z(ncf[1:(i+1)],value[i+1]))
}
...
par(mfrow=c(3,1))
plot(zoo(100*adj_cap_rate,year),type='l',col='red',main="Cap Rate", ylab="", xlab = "")
plot(value,type='l',col='blue',main="Value",ylab="",xlab="")
plot(zoo(c(NA,delta_value),year),type='l',col='red',main='Income',ylab="",xlab="Year",ylim=c(-10,50))
lines(zoo(c(NA,noi[-16]),year),col='blue')
lines(zoo(c(NA,tr),year),col='green')
legend('topright',legend=c('NOI','Change in Value','Total Income'),col=c('blue','red','green'),lwd=2)
```

```{r, echo=FALSE, fig.height=4}
plot(100*twr,type='l',col='red',main='Returns',ylab='Percent',xlab='Year')
lines(100*irrs,type='l',col='green')
lines(100*cash_return,type='l',col='blue')
legend('topright',legend=c('Inception IRR','Annual TWR','Cash on Cost'),col=c('green','red','blue'),lwd=2)
```
